    ...
    navigator.clipboard.readText()
      .then(text => {
        if (text && text.includes('tiktok.com')) {
          ...
          toast.error('Clipboard does not contain a TikTok URL');
        }
      })
      .catch(() => {
      ...
      <div className="flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-2">
        <div className="relative flex-1">
          ...
          <Button 
            type="button" 
            onClick={handlePaste}
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-transparent hover:bg-white/10 text-tiktok-primary h-8 px-3"
            disabled={isLoading}
          >
            Paste
          </Button>
